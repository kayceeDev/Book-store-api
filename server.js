...
const server = http.createServer((req, res) => {
  ...
  if (req.url === "/api/users" && req.method === "GET") {
    ...
    req.url.match(/\/api\/users\/books\/\w+/) &&
    req.method === "POST"
  ) {
    // @POST api/users/books Adds a book to library ( only registered users can do that)
    const userId = req.url.split("/")[4];
    addBookToStore(req, res, userId);
  } else if (
    req.url.match(/\/api\/books\/users\/\w+\/\w+/) &&
    req.method === "PUT"
  ) {
